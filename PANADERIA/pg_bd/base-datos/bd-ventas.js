...
export async function registrarVentaDesdePedido(pedido) {
  ...
  const itemsValidos = productos
    ...
    .filter((item) => Number.isInteger(item.idProductoReal) && item.idProductoReal > 0);
  ...
  if (!idSucursal) {
    console.warn(`Pedido #${pedido.id_pedido}: no se encontró ninguna sucursal, no se generó Venta.`);
    return null;
  }
